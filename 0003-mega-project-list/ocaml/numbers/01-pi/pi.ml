 ...
 * limited precision and (b) just doing the 25 iterations needed to get PI
 * to the number of iterations a float can support.
 *
 * It strikes me that I'd be able to implement this more neatly in Haskell
 * by using a lazy list mapped by iter and summed.
 *)
