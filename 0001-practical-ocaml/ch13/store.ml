...
module type ACCOUNT = functor (S : STORE) -> sig
  val get_account_id : string -> string
  val get_contact_email : string -> string
end

module type ACCOUNT_priv = functor (A : ACCT_NUM) (S : STORE) -> sig
...
functor
  ...
  struct
    ...
    let create x y =
      ...
      if exists then raise (Account_error "Username already exists")
      else S.add { id = A.next (); username = x; contact_email = y }
