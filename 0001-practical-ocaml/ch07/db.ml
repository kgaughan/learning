...
let gen_account acdb accname max_holding init_pos =
  if Hashtbl.mem acdb accname then
    ...
    Hashtbl.add acdb accname
      { name = accname; max_ind_holding = max_holding; pos = init_pos }

let account_buy (symb, price) quant acc =
  {
    acc with
    pos = { symbol = symb; holding = quant; pprice = price } :: acc.pos;
  }
...
let account_sell (symb, price) acc =
  let rec seller sym prc pos_list soldq soldv newposlst =
    ...
    | h :: t ->
        if h.symbol = sym then
          seller sym prc t (soldq + h.holding)
            ((float_of_int h.holding *. (prc -. h.pprice)) +. soldv)
            newposlst
        else seller sym prc t soldq soldv (h :: newposlst)
  in
...
let symbols_in_account acc = List.map (fun x -> x.symbol) acc.pos

let value_at_purchase acc =
  List.fold_left
    (fun y x -> (float_of_int x.holding *. x.pprice) +. y)
    0. acc.pos

let current_value acc cur_prices =
  List.fold_left
    (fun y x -> (float_of_int x.holding *. List.assoc x.symbol cur_prices) +. y)
    0. acc.pos
...
let total_pandl accdb cur_prices =
  List.fold_left ( +. ) 0.
    (Hashtbl.fold (fun x y z -> profit_and_loss y cur_prices :: z) accdb [])
...
let percent_holding acc cur_prices =
  ...
  List.map
    (fun x ->
      ( x.symbol,
        float_of_int x.holding *. List.assoc x.symbol cur_prices /. curval ))
    acc.pos
...
let accounts_holding symb accdb =
  Hashtbl.fold
    (fun x y z -> if contains_symbol symb y.pos then x :: z else z)
    accdb []
...
let summary_stats items =
  ...
  let std_dev =
    sqrt
      (List.fold_left (fun y n -> ((n -. mean) *. (n -. mean)) +. y) 0. items
...
let top_10 db new_prices =
  let lst =
    List.sort
      (fun (m, n) (x, y) -> compare y n)
      (Hashtbl.fold (fun x y z -> (x, profit_and_loss y new_prices) :: z) db [])
  in
  top_n lst [] 10
...
let print_top_report title lst =
  let rec toprep buf items =
    match items with
    | [] ->
        let sum, mn, med, stdev =
          summary_stats (List.map (fun (x, y) -> y) lst)
        in
...
let position_of_string s =
  Scanf.sscanf s "%s %i %0.4f" (fun x y z ->
...
let export_accounts db filename =
  ...
  Hashtbl.iter
    (fun key data -> Printf.fprintf oc "%s\n" (string_of_account data))
    db;
  close_out oc
...
let account_of_string str =
  let rec build_pos sb accum =
    let getnextch =
      try
        Scanf.bscanf sb "%c" (fun issep ->
            match issep with
            | '|' ->
                Scanf.bscanf sb "%s %i %f" (fun x y z ->
                    Some { symbol = x; holding = y; pprice = z })
            | _ -> raise (Invalid_argument "Malformed position"))
      with End_of_file -> None
    in
    match getnextch with None -> accum | Some p -> build_pos sb (p :: accum)
  in
...
let import_accounts dstore filename =
  ...
  let rec iaccts chan store =
    let newacc =
      try Some (account_of_string (input_line ic)) with End_of_file -> None
    in
    match newacc with
    | None -> ()
    | Some p ->
        Hashtbl.add store p.name p;
        iaccts ic store
  in
...
let rec gen_random_pos_list len accu price_list =
  ...
  | _ ->
      let sym, price =
        List.nth price_list (Random.int (List.length price_list))
      in
      gen_random_pos_list (len - 1)
        ({ symbol = sym; holding = Random.int 1000; pprice = price } :: accu)
        price_list
...
let rec populate_db rand_cands store current_prices =
  ...
  | 0 -> ()
  | _ ->
      let newacc = gen_random_account current_prices in
