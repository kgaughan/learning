...
let quote x =
  ...
  | Mailto (Email (m, n)) ->
      ...
      let _ = String.iter (fun x -> Buffer.add_string acbuf (trans_mailto x)) m in
      let _ = Buffer.add_string acbuf "@" in
      let _ = String.iter (fun x -> Buffer.add_string acbuf (trans_mailto x)) n in
      Buffer.contents acbuf
  ...
  Scanf.bscanf b "%s@:" (
    fun x ->
      ...
      | "http" ->
          ...
          let path = Scanf.bscanf b "%n" (fun x -> String.sub m (x - 1) ((String.length m) - (x - 1))) in
          Http (web, ("/" ^ path))
      | "mailto" ->
          let em = Scanf.bscanf b "%s@@" (fun x -> x) in
          let dom = Scanf.bscanf b "%n" (fun x -> String.sub m x ((String.length m) - x)) in
          Mailto (Email (em, dom))
