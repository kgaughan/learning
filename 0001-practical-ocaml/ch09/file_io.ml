...
let catfile filename =
  ...
  try print_all_lines in_file with End_of_file -> close_in in_file
...
let random_catfile filename =
  ...
  let actually_input =
    ...
    input in_file str_buf 0 segment
  in
  close_in in_file;
  output_string stdout str_buf

let is_dir x = (Unix.stat x).Unix.st_kind = Unix.S_DIR
...
let ls x =
  ...
  | true ->
      ...
      let rec buildlist d acc =
        try buildlist d (acc @ [ Unix.readdir d ]) with End_of_file -> acc
      in
      buildlist udir []
